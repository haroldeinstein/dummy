function RepManager() {

  var manager = this;
  this.reps = new RepsCollection();
  this.reps.fetch();
}

RepManager.prototype = {
  showNewModal: function($elem) {
    var modal = this.getNewHTML();
    var $modal = $(modal);
    $('body').append($modal);

    $modal.show();

    var modalHeight = $modal.outerHeight();
    var modalWidth = $modal.outerWidth();
    var offsetLeft = $elem.offset().left;
    var offsetTop = $elem.offset().top;

    $modal.hide();

    $modal.css({
      top : offsetTop,
      left : offsetLeft - modalWidth - 20
    });

    $modal.show();

  },

  showEditModal: function(id, $elem) {
    var rep = this.reps.get(id).toJSON();
    var modal = this.getEditHTML(rep);
    var $modal = $(modal);
    $('body').append($modal);

    $modal.show();

    var modalHeight = $modal.outerHeight();
    var modalWidth = $modal.outerWidth();
    var offsetLeft = $elem.offset().left;
    var offsetTop = $elem.offset().top;

    $modal.hide();

    $modal.css({
      top : offsetTop,
      left : offsetLeft - modalWidth - 20
    });

    $modal.show();

  },

  hideEditModal: function() {

  },

  getEditHTML: function(repLocation) {

    var html = "";
    html = '<div id="rep-modal-container">';
    html += '<div id="rep-modal">';

    html += '<label>location</label>';
    html += '<input type="text" value="' + repLocation.location + '"></input><br>';

    html += '<label>name</label>';
    html += '<input type="text" value="';
    if (repLocation.reps[0])
      html += repLocation.reps[0].name;
    html += '"></input><br>';

    html += '<label>name</label><br>';
    html += '<input type="text" value="';
    if (repLocation.reps[1])
      html += repLocation.reps[1].name;
    html += '"></input><br>';

    html += '</div>';
    html += '</div>';

    return html;
  },

  getNewHTML: function() {
    var html = '<div id="rep-modal-container">';
    html += '<div id="rep-modal">';

    html += '<label>location</label>';
    html += '<input type="text" value=""></input>';

    html += '<label>name</label>';
    html += '<input type="text" value=""></input>';

    html += '<label>name</label>';
    html += '<input type="text"></input>';

    html += '</div>';
    html += '</div>';

    return html;
  }
}
